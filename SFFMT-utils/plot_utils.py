...
def plot_confusion_matrix(y_ture, y_pred, norm=True, task=1, fig_name='test'):
    ...
    ax.set_title('FCD-task' if task == 1 else 'FSD-task', fontdict=font1)  # 标题
...
def t_sne(input_data, input_label, classes, task, fig_name=None, labels=None, n_dim=2):
    ...
    if labels is None:
        if classes == 6 and task == 1:
            labels = ['A', 'C', '01', '03', '05', '08']  # three types
        elif classes == 4 and task == 1:
            labels = ['NC', 'IF', 'OF', 'RF']  # four types
        elif classes == 4 and task == 2:
            labels = ['A', 'C', 'B', 'G']  # four types 'F0', 'F1', 'F2', 'F3'
    ...
    if classes == 6 and task == 1:
        ...
        ax.scatter(type4[:, 0], type4[:, 1], s=100, color=palette[4], alpha=0.8, marker=mark[4], label=labels[4])
        ax.scatter(type5[:, 0], type5[:, 1], s=100, color=palette[5], alpha=0.8, marker=mark[5], label=labels[5])
        ax.set_xlim(-0.1, 1.3)
        ax.set_ylim(-0.1, 1.3)
